...
//      $records = null; // Free memory.
  ...
  /**
   * @param string[] $tableNames string or string[]
   */
  public static function truncate($tableNames)
  {
    ...
    foreach ($tableNames as $tableName) {
      $command->truncateTable($tableName)->execute();
    ...
    try {
      ...
      while (($data = fgetcsv($handle)) !== FALSE) {
        if ($data[0] === self::TABLE_MARKER) {
          ...
          $modelClassName = 'app\\models\\' . $data[1];
          ...
          foreach ($attributes as $index => $attribute) {
            if ($attribute) {
              $model->$attribute = (!isset($data[$index]) ||
                                   $data[$index] === 'NULL' || $data[$index] === '') ? NULL : $data[$index];
            }
          }
          // Save record.
          if (!$model->save()) {
